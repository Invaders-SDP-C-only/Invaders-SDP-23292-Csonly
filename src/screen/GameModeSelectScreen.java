...
public class GameModeSelectScreen extends Screen {
    ...
    private static class BackgroundEnemy extends Entity {
        ...
        public BackgroundEnemy(int positionX, int positionY, int speed, DrawManager.SpriteType spriteType) {
    ...
    public GameModeSelectScreen(final int width, final int height, final int fps) {
    ...
    protected void update() {
        ...
        if (this.enemySpawnCooldown.checkFinished()) {
            ...
            if (Math.random() < ENEMY_SPAWN_CHANCE) {
                DrawManager.SpriteType[] enemyTypes = {
                        DrawManager.SpriteType.EnemyShipA1,
                        DrawManager.SpriteType.EnemyShipB1,
                        DrawManager.SpriteType.EnemyShipC1
                };
                DrawManager.SpriteType randomEnemyType = enemyTypes[random.nextInt(enemyTypes.length)];
        ...
        if (inputManager.isKeyDown(KeyEvent.VK_ENTER)
            ...
            if (this.selection == 0) this.selectedMode = TYPE_CLASSIC;
            else if (this.selection == 1) this.selectedMode = TYPE_SANDBOX;
            else this.selectedMode = TYPE_CANCEL;
            this.isRunning = false;
            return;
        }
    ...
    private void draw() {
        ...
        drawManager.drawGameModeSelection(this, selection);
